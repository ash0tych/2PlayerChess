...
class Pawn(ChessFigure):
    ...
    def clicked(self, screen_name, board, xy):
        moves = self.get_moves(board, (xy[0] // 80, xy[1] // 80))
        for pos in moves:
...
class ChessBoard(object):
    def __init__(self):
        ...
        self.board[2][1] = Pawn(Colors.WHITE)
    ...
    def check_click(self, pos):
        return type(self.board[pos[1] // 80][pos[0] // 80]) != EmptyFigure

    def draw_borders(self, screen_name, pos):
            self.board[pos[1] // 80][pos[0] // 80].clicked(screen_name, self.board, pos)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:  # Better to seperate to a new if statement aswell, since there's more buttons that can be clicked and makes for cleaner code.
            if event.button == 1 and c_board.check_click(event.pos):
                screen.fill((255, 255, 255))
                c_board.draw_all(screen)
                c_board.draw_borders(screen, event.pos)
                pygame.display.update()
